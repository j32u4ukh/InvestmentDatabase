<?php
	...
	class Database{
		...
		private function connectPDO($server, $user, $password, $database){		
			try {
				$this->db = new PDO("mysql:host=$server; charset=utf8mb4; dbname=$database", self::user, self::password);
		...
		public function update($table = null, $data_array = null, $key_column = null, $id = null) {
			...
			$this->sql = "UPDATE $table SET $setting_list WHERE $key_column = :$key_column";
			$result = $this->db->prepare($this->sql);                       
			$result->execute($data_array);
		}
